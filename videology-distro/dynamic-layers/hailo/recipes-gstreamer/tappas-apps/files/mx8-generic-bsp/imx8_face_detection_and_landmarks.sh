...
if [[ $input_source =~ "/dev/video" ]]; then
    source_element="v4l2src device=$input_source name=src_0 ! video/x-raw, format=YUY2, width=$width, height=$height, framerate=30/1 ! videoconvert qos=false n-threads=4"
    internal_offset=false
else
    source_element="filesrc location=$input_source name=src_0 ! decodebin ! queue ! videoconvert qos=false n-threads=4"
