...
static int sensor_open(struct v4l2_subdev* sd, struct v4l2_subdev_fh* fh)
{
    ...
    struct v4l2_mbus_framefmt* try_img_fmt = v4l2_subdev_get_try_format(sd, fh->state, 0);
...
static int sensor_set_ctrl(struct v4l2_ctrl* ctrl)
{
    ...
    switch (ctrl->id) {
    default:
        dev_info(&sensor->pdev->dev, "ctrl(id:0x%x,val:0x%x) is not handled\n", ctrl->id, ctrl->val);
        ret = -EINVAL;
        break;
    }
...
static int sensor_enum_mbus_code(struct v4l2_subdev* sd, struct v4l2_subdev_state* sd_state, struct v4l2_subdev_mbus_code_enum* code)
{
...
static int sensor_enum_frame_size(struct v4l2_subdev* sd, struct v4l2_subdev_state* sd_state, struct v4l2_subdev_frame_size_enum* fse)
{
    if (fse->index)
        return -EINVAL;
    if (fse->pad)
...
static int sensor_get_pad_format(
    struct v4l2_subdev* sd, struct v4l2_subdev_state* sd_state, struct v4l2_subdev_format* fmt)
{
    struct sensor* sensor = to_sensor(sd);
    ...
    if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
        struct v4l2_mbus_framefmt* try_fmt = v4l2_subdev_get_try_format(&sensor->sd, sd_state, fmt->pad);
        fmt->format = *try_fmt;
    } else {
...
static int sensor_get_selection(struct v4l2_subdev* sd, struct v4l2_subdev_state* sd_state, struct v4l2_subdev_selection* sel)
{
    struct sensor* sensor = to_sensor(sd);
...
static int sensor_init_controls(struct sensor* sensor)
{
    ...
    v4l2_ctrl_new_std(ctrl_hdlr, &sensor_ctrl_ops, V4L2_CID_PIXEL_RATE, 1, 1, 1, 1);

    /* Initial vblank/hblank/exposure parameters based on current mode */
    ctrl = v4l2_ctrl_new_std(ctrl_hdlr, &sensor_ctrl_ops, V4L2_CID_VBLANK, 1, 1, 1, 1);
    if (ctrl)
        ctrl->flags |= V4L2_CTRL_FLAG_READ_ONLY;

    ctrl = v4l2_ctrl_new_std(ctrl_hdlr, &sensor_ctrl_ops, V4L2_CID_HBLANK, 1, 1, 1, 1);
    if (ctrl)
        ctrl->flags |= V4L2_CTRL_FLAG_READ_ONLY;

    ctrl = v4l2_ctrl_new_std(ctrl_hdlr, &sensor_ctrl_ops, V4L2_CID_EXPOSURE, 1, 1, 1, 1);
    if (ctrl)
        ctrl->flags |= V4L2_CTRL_FLAG_READ_ONLY;
...
static int sensor_probe(struct platform_device *pdev)
{
    ...
    /* initialize name */
    snprintf(sensor->sd.name, sizeof(sensor->sd.name), "%s", pdev->dev.driver->name);
    ...
    sensor->fmt.quantization = V4L2_MAP_QUANTIZATION_DEFAULT(true, sensor->fmt.colorspace, sensor->fmt.ycbcr_enc);
...
static const struct of_device_id sensor_dt_ids[]
    = { { .compatible = "scailx,dummy-csi2-sensor" }, { /* sentinel */ } };
